...
files_by_size.each do | size, filepath |
   ...
   if direction > 0 
      ...
      output_filename = "#{output_directory}/#{datanodes[index].chomp}.xdata.data-tactics-corp.com.txt"
   else
      #puts "datanode count - datanode count: #{datanode_count} index: #{index} from_back: #{datanode_count-index}\n"
      output_filename = "#{output_directory}/#{datanodes[datanode_count-index-1].chomp}.xdata.data-tactics-corp.com.txt"
   end
